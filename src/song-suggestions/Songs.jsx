...
export default function Songs({getAccessToken, chosenSongs, setChosenSongs, setStep}) {
...
if (song.length > 2) {
  ...
  return (
    ...
    {songSuggestions && song.length > 2 ?
      <Suggestions>
        {songSuggestions.map((suggestion) => <Suggestion onClick={(e) => setSongs(suggestion)}><ArtistImg src={suggestion.images[0].url} />{suggestion.name}</Suggestion>)}
      </Suggestions>
      : null
    }
      </Label>
